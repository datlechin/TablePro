...
- **Always use team agents** for implementation work. Use the Agent tool to delegate to specialized agents: `general-purpose` (implementation), `feature-dev:code-architect` (design), `feature-dev:code-explorer` (analysis), `feature-dev:code-reviewer` (review), `code-simplifier:code-simplifier` (cleanup). Use the Skill tool for `/feature-dev` (guided feature development).
