...
TablePro ships with 2 built-in database driver plugins (MySQL, PostgreSQL) and 3 built-in export plugins (CSV, JSON, SQL). Additional plugins are available from the plugin registry and can be installed from the Browse tab or downloaded automatically when you select a database type:
